...
<div class="space-y-8 animate-fade-in-up">
    ...
    <div class="glass-panel rounded-[2rem] shadow-2xl p-8 relative overflow-hidden transition-all duration-500 hover:shadow-blue-900/5">
            ...
            <div class="lg:col-span-8">
                <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 lg:grid-cols-5 gap-3">
                    ...
                    <div class="space-y-1.5 group">
                        <label class="block text-xs font-black text-slate-400 uppercase tracking-widest group-hover:text-indigo-500 transition-colors"><?= $label ?></label>
                        <div class="relative">
        ...
        <div class="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 mt-8 pt-6 border-t border-slate-100/60">
            <div class="flex flex-wrap items-center gap-4 w-full sm:w-auto">
                ...
                <div class="flex items-center gap-2 text-xs">
                    ...
                    <span class="px-2 py-1 rounded-md bg-slate-100 text-slate-600 font-semibold truncate max-w-[150px] sm:max-w-none" id="activeFiltersText">Default View</span>
                </div>
            </div>
            
            <div class="flex flex-col sm:flex-row items-stretch sm:items-center gap-3 w-full sm:w-auto">
                <a href="#" id="exportCsv" class="group relative px-6 py-3 bg-white border border-slate-200 text-slate-600 text-xs font-black uppercase tracking-widest rounded-xl hover:bg-slate-50 hover:border-slate-300 hover:shadow-lg hover:-translate-y-0.5 transition-all active:scale-95 flex items-center justify-center gap-2 overflow-hidden">
                ...
                <button type="button" id="refreshReport" class="group relative px-8 py-3 bg-slate-800 text-white text-xs font-black uppercase tracking-widest rounded-xl shadow-lg shadow-slate-900/20 hover:bg-slate-700 hover:shadow-xl hover:shadow-slate-900/30 hover:-translate-y-0.5 transition-all active:scale-95 flex items-center justify-center gap-3 overflow-hidden">
                    <svg class="w-4 h-4 group-hover:animate-spin" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 4v5h.582m15.356 2A8.001 8.001 0 004.582 9m0 0H9m11 11v-5h-.581m0 0a8.003 8.003 0 01-15.357-2m15.357 2H15"></path></svg>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-12 gap-8 animate-fade-in-up animation-delay-200">
        ...
        <div class="lg:col-span-5 flex flex-col h-full">
            <div class="glass-card rounded-[2.5rem] p-8 h-full relative group">
                ...
                <div class="relative min-h-[300px] h-auto w-full flex items-center justify-center">
...
(function() {
    ...
    function renderChart(data, type) {
        ...
        if (useHorizontal) {
            ...
            // Allow CSS to handle it (min-h-[300px]) or fallback
            // We set a style only if we need to force it, otherwise clear it to let CSS work
            chartContainer.style.height = ''; 
            if (chartContainer.offsetHeight < 300) {
     ...
     * This is critical for user-controlled data like positions, tasks, courses, etc.
     */
    ...
    function renderTables(sections) {
        ...
        sections.forEach((sec, idx) => {
            ...
            let headers = '';
            if (sec.headers) {
                // Headers are from backend constants, but escape for safety
                headers = sec.headers.map(h => 
                    `<th class="px-5 py-3 text-left text-[9px] font-black text-slate-500 uppercase tracking-widest whitespace-nowrap">${escapeHtml(h)}</th>`
                ).join('');
            }
            
            let rows = '';
            (sec.rows || []).forEach((row) => {
                // SECURITY: Escape all cell values to prevent XSS from user-controlled data
                const cells = row.map((cell, cIdx) => {
                    const style = cIdx === 0 ? 'font-bold text-slate-800' : 'text-slate-600 font-medium';
                    return `<td class="px-5 py-3 ${style} whitespace-nowrap">${escapeHtml(cell)}</td>`;
                }).join('');
            ...
            html += `
                <div class="bg-white rounded-3xl border border-slate-100 shadow-sm overflow-hidden mb-8 last:mb-0 group hover:shadow-md transition-shadow">
                    <div class="px-6 py-4 border-b border-slate-50 bg-slate-50/30 flex items-center gap-3">
                        ...
                        <h4 class="font-bold text-slate-800 text-sm">${escapeHtml(sec.title)}</h4>
                    </div>
